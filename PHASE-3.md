...
**Integration Test Temeli (Bölüm 17 Seviyesi)**
- [ ] Neden integration test burada başlıyor? — Express route, middleware ve DB birlikte test edilmeli
- [ ] Test ortamı için ayrı `.env.test` dosyası
- [ ] Test DB — her test suite öncesi temizle (`beforeEach` / `beforeAll`)
- [ ] supertest ile HTTP seviyesinde request → response testi
- [ ] Auth gerektiren route testi — token üret, header'a ekle, response kontrol et
- [ ] Hata senaryosu testi — 400, 401, 404 response'larını doğrula
- [ ] Vitest veya Jest ile test runner kurulumu
...
**Environment Stratifikasyonu**
- [ ] 4 ortam ayrımı: `development` / `test` / `staging` / `production`
- [ ] Her ortamın farklı ihtiyaçları:
  - [ ] `development`: verbose log, hot reload, local DB
  - [ ] `test`: in-memory veya test DB, log kapalı, deterministik seed
  - [ ] `staging`: production mirror, gerçek servisler ama gerçek veri yok
  - [ ] `production`: minimal log, gerçek servisler, secret manager
- [ ] `NODE_ENV` ile ortam kontrolü — ama tek başına yetmez
- [ ] Her ortam için `.env.development`, `.env.test`, `.env.staging`, `.env.production`
- [ ] CI/CD'de ortam değişkenlerini nasıl inject edersin?
- [ ] Staging ortamının neden önemli olduğu — production'dan önce son kalite kapısı
...
**System Design Mini Sorusu — Veritabanı**
> Bu soruları şimdi tam cevaplamak zorunda değilsin. Düşün, taslak çiz, internette araştır.
- [ ] 10 milyon kullanıcın olsaydı hangi tablolara index eklerdin?
- [ ] Kullanıcı tablosu çok büyüdüğünde (100M satır) pagination nasıl değişir?
- [ ] Aynı anda 1000 kişi aynı ürünü satın almaya çalışsa (stok azaltma) ne olur? Bunu nasıl çözersin?
- [ ] Read-heavy bir uygulama için (blog, haber sitesi) DB mimarisi nasıl olurdu?
...
**System Design Mini Sorusu — Queue ve Worker**
> Düşün ve taslak çiz.
- [ ] 1 worker ile 10.000 email göndermen gerekse ne kadar sürer? Kaç worker gerekir?
- [ ] Worker process çöktüğünde yarıda kalan job'a ne olur? Bunu nasıl önlersin?
- [ ] Email servisi (Resend, SendGrid) geçici olarak down olsa sistem nasıl davranmalı?
- [ ] Aynı job'ın iki kez çalışması (at-least-once delivery) ne zaman sorun yaratır, nasıl önlersin?
...
Async JS, Node.js, HTTP, Express, NestJS mimarisi, SQL, PostgreSQL, ORM, Redis, Queue ve Docker. Bu faz backend'in tüm çekirdeği — burada öğrenilen her şey production'da her gün kullanılır.

### Bu faz senden sonra ne açtı?
Artık çalışan, güvenli, test edilebilir bir API yazabiliyorsun. Faz 4'te bunu "üretime hazır" hale getiriyorsun: auth akışları, güvenlik, kapsamlı test, observability. Faz 5'te frontend bu API'yi tüketecek — ve neyin neden böyle çalıştığını anlayarak yazacaksın.

### Geliştiricilerin %80'inin yaptığı hata
- Express'i öğrenip NestJS'e geçince "neden bu kadar karmaşık?" demek — DI ve modül sistemini anlamadan geçmek
- SQL öğrenmeden Prisma'ya atlamak — ORM'in ürettiği sorguyu hiç görmemek
- `.env` dosyasını Git'e commit etmek
- N+1 sorguyu fark etmemek — "yavaş ama çalışıyor" demek
- Docker'ı sadece "bir şekilde çalıştırdım" seviyesinde bırakmak, neden çalıştığını anlamamak
- Queue'yu "sonra eklerim" deyip hiç eklememek

### Açık kaynak okuma görevi
NestJS'in resmi örnek projelerine git: `nestjs/nest` → `sample/` klasörü. `01-cats-app` veya `23-graphql-code-first` klasörlerini aç. Module/Controller/Service yapısını gör. Kendi yazdığınla karşılaştır. Nasıl organize etmişler?
